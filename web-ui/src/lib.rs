...
pub fn app() -> Html {
    ...
    use crate::messages::{get_message_from_result, get_message_class, MessageLevel};
                ...
                match convert_plantuml(text, ImageFormat::Svg).await {
                    Ok((bytes, result)) => {
                        ...
                        match String::from_utf8(bytes) {
                            Ok(svg_text) => {
                                let data_url = format!("data:image/svg+xml;charset=utf-8,{}", 
                    ...
                    Err(e) => {
                        ...
                        match e {
                            ApiError::ProcessError { code, level, context } => {
